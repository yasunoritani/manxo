# Max 9 - Claude Desktop 状態同期プロトコル仕様

## 概要

本ドキュメントでは、Max 9とClaude Desktop MCPサーバー間の状態同期プロトコルを詳細に定義します。このプロトコルは、Max 9の現在の状態をMCPサーバーに反映し、一貫性のある操作を可能にします。

## 1. 状態モデル

Max 9の状態は以下の階層構造でモデル化されます：
...
Max 9での状態変更はMCPサーバーに以下の形式で通知されます：
...
MCPサーバーはMax 9の現在の状態を以下の形式で要求できます：
...
Max 9は状態同期リクエストに対して以下の形式でレスポンスします：
